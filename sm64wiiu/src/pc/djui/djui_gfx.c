...
// Donor DJUI uses extended display-list helpers that are not present in this
// Wii U tree yet. These shims preserve render-path behavior safely.
#ifndef gsSPTextureAddrDjui
#define gsSPTextureAddrDjui(_enabled) gsDPPipeSync()
#endif
#ifndef gsSPVertexNonGlobal
#define gsSPVertexNonGlobal(_vtx, _count, _first) gsSPVertex(_vtx, _count, _first)
#endif
#ifndef gsSPExecuteDjui
#define gsSPExecuteDjui(_op) gsDPPipeSync()
#endif
#ifndef G_TEXOVERRIDE_DJUI
#define G_TEXOVERRIDE_DJUI 0
#endif
#ifndef G_TEXCLIP_DJUI
#define G_TEXCLIP_DJUI 0
#endif
#ifndef gSPVertexNonGlobal
#define gSPVertexNonGlobal(_pkt, _vtx, _count, _first) gSPVertex(_pkt, _vtx, _count, _first)
#endif
#ifndef gSP2TrianglesDjui
#define gSP2TrianglesDjui(_pkt, _v00, _v01, _v02, _f0, _v10, _v11, _v12, _f1) \
    gSP2Triangles(_pkt, _v00, _v01, _v02, _f0, _v10, _v11, _v12, _f1)
#endif
#ifndef gDPSetTextureClippingDjui
#define gDPSetTextureClippingDjui(_pkt, _x1, _y1, _x2, _y2) ((void)0)
#endif
#ifndef gDPLoadTextureBlockWithoutTexture
#define gDPLoadTextureBlockWithoutTexture(_pkt, _tex, _fmt, _siz, _w, _h, _pal, _cms, _cmt, _masks, _maskt, _shifts, _shiftt) ((void)0)
#endif
#ifndef gDPSetTextureOverrideDjui
#define gDPSetTextureOverrideDjui(_pkt, _tex, _wPow2, _hPow2, _fmt, _siz)                    \
    do {                                                                                       \
        if ((_siz) == G_IM_SIZ_32b) {                                                          \
            gDPLoadTextureBlock(_pkt, _tex, G_IM_FMT_RGBA, G_IM_SIZ_32b,                      \
                                (1U << (_wPow2)), (1U << (_hPow2)),                           \
                                0, G_TX_CLAMP, G_TX_CLAMP, 0, 0, 0, 0);                       \
        } else {                                                                               \
            gDPLoadTextureBlock(_pkt, _tex, G_IM_FMT_RGBA, G_IM_SIZ_16b,                      \
                                (1U << (_wPow2)), (1U << (_hPow2)),                           \
                                0, G_TX_CLAMP, G_TX_CLAMP, 0, 0, 0, 0);                       \
        }                                                                                      \
    } while (0)
...
void djui_gfx_render_texture(const Texture* texture, u32 w, u32 h, u8 fmt, u8 siz, bool filter) {
    ...
    if (fmt != G_IM_FMT_RGBA) {
        LOG_ERROR("djui_gfx_render_texture: unsupported texture fmt=%u", fmt);
        return;
    }

    if (siz == G_IM_SIZ_32b) {
        gDPSetTextureOverrideDjui(gDisplayListHead++, texture, djui_gfx_power_of_two(w), djui_gfx_power_of_two(h), G_IM_FMT_RGBA, G_IM_SIZ_32b);
    } else {
        gDPSetTextureOverrideDjui(gDisplayListHead++, texture, djui_gfx_power_of_two(w), djui_gfx_power_of_two(h), G_IM_FMT_RGBA, G_IM_SIZ_16b);
    }
...
void djui_gfx_render_texture_tile(const Texture* texture, u32 w, u32 h, u8 fmt, u8 siz, u32 tileX, u32 tileY, u32 tileW, u32 tileH, bool filter, bool font) {
    ...
    if (fmt != G_IM_FMT_RGBA) {
        LOG_ERROR("djui_gfx_render_texture_tile: unsupported texture fmt=%u", fmt);
        return;
    }
...
bool djui_gfx_add_clipping_specific(struct DjuiBase* base, f32 dX, f32 dY, f32 dW, f32 dH) {
    ...
    // Wii U float-space conversion can produce tiny over/under-shoots that
    // otherwise wrap when converted to u8 clip percentages.
    dClipX1 = clamp(dClipX1, 0.0f, 1.0f);
    dClipY1 = clamp(dClipY1, 0.0f, 1.0f);
    dClipX2 = clamp(dClipX2, 0.0f, 1.0f);
    dClipY2 = clamp(dClipY2, 0.0f, 1.0f);

    if ((dClipX1 != 0) || (dClipY1 != 0) || (dClipX2 != 0) || (dClipY2 != 0)) {
        gDPSetTextureClippingDjui(gDisplayListHead++,
                                  (u8)roundf(dClipX1 * 255.0f),
                                  (u8)roundf(dClipY1 * 255.0f),
                                  (u8)roundf(dClipX2 * 255.0f),
                                  (u8)roundf(dClipY2 * 255.0f));
    }
