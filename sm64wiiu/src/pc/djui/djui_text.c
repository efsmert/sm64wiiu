...
static void djui_text_render_single_char(struct DjuiText* text, char* c) {
    ...
    // Give glyph edges a tiny clip margin to avoid Wii U per-char
    // over-clipping artifacts on baseline/descender pixels.
    if (djui_gfx_add_clipping_specific(&text->base,
                                       dX - clipPadding,
                                       dY - clipPadding,
                                       dW + clipPadding * 2.0f,
                                       dH + clipPadding * 2.0f)) {
