...
#else
    bool clientAvailable = network_client_available();
    struct DjuiThreePanel* panel = djui_panel_menu_create(DLANG(JOIN, JOIN_TITLE), false);
    struct DjuiBase* body = djui_three_panel_get_body(panel);
    {
        struct DjuiButton* publicButton = djui_button_create(body, DLANG(JOIN, PUBLIC_LOBBIES), DJUI_BUTTON_STYLE_NORMAL, djui_panel_join_public_lobbies);
        struct DjuiButton* privateButton = djui_button_create(body, DLANG(JOIN, PRIVATE_LOBBIES), DJUI_BUTTON_STYLE_NORMAL, djui_panel_join_private_create);
        struct DjuiButton* directButton = djui_button_create(body, DLANG(JOIN, DIRECT), DJUI_BUTTON_STYLE_NORMAL, djui_panel_join_direct_create);
        struct DjuiButton* backButton = djui_button_create(body, DLANG(MENU, BACK), DJUI_BUTTON_STYLE_BACK, djui_panel_menu_back);

        djui_base_set_enabled(&publicButton->base, clientAvailable);
        djui_base_set_enabled(&privateButton->base, clientAvailable);
        djui_base_set_enabled(&directButton->base, clientAvailable);

        if (!clientAvailable) {
            struct DjuiText* text = djui_text_create(body, DLANG(NOTIF, LOBBY_JOIN_FAILED));
            djui_base_set_size_type(&text->base, DJUI_SVT_RELATIVE, DJUI_SVT_ABSOLUTE);
            djui_base_set_size(&text->base, 1.0f, 32.0f);
            djui_base_set_color(&text->base, 220, 220, 220, 255);
            djui_text_set_alignment(text, DJUI_HALIGN_CENTER, DJUI_VALIGN_CENTER);
            djui_cursor_input_controlled_center(&backButton->base);
        }
    }
