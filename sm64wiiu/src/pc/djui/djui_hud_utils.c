...
void djui_hud_reset_viewport(void) {
    ...
    gSPViewport(gDisplayListHead++, &D_8032CF00);
#else
...
f32 get_current_fov() {
    return get_first_person_enabled() ? gFirstPersonCamera.fov : (sFOVState.fov + sFOVState.fovOffset);
}
...
bool djui_hud_world_pos_to_screen_pos(Vec3f pos, VEC_OUT Vec3f out) {
    ...
    Mat4 cameraMtx;
    mtxf_lookat(cameraMtx, gLakituState.pos, gLakituState.focus, gLakituState.roll);
    hud_rotate_and_translate_vec3f(pos, &cameraMtx, out);
    if (out[2] >= 0.0f) {
        return false;
    }
