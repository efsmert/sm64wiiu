...
static struct DjuiBase* djui_cursor_find_first_interactable(struct DjuiBase* base) {
    ...
    while (child != NULL) {
        struct DjuiBase* found = djui_cursor_find_first_interactable(child->base);
        if (found != NULL) {
            return found;
        }
        child = child->next;
...
static void djui_cursor_resolve_input_base(void) {
    if (sInputControlledBase != NULL || gDjuiRoot == NULL) {
        return;
    }
    sInputControlledBase = djui_cursor_find_first_interactable(&gDjuiRoot->base);
}
...
static void djui_cursor_update_position(void) {
    ...
    djui_cursor_resolve_input_base();
