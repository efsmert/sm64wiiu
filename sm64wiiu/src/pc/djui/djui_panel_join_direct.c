...
void djui_panel_join_direct_do_join(struct DjuiBase* caller) {
    ...
    if (!network_client_available()) {
        djui_panel_join_message_error(DLANG(NOTIF, LOBBY_JOIN_FAILED));
        return;
    }
...
void djui_panel_join_direct_create(struct DjuiBase* caller) {
    ...
    struct DjuiBase* body = djui_three_panel_get_body(panel);
    {
        ...
        if (!clientAvailable) {
            struct DjuiText* text2 = djui_text_create(body, DLANG(NOTIF, LOBBY_JOIN_FAILED));
            djui_base_set_size_type(&text2->base, DJUI_SVT_RELATIVE, DJUI_SVT_ABSOLUTE);
            djui_base_set_size(&text2->base, 1.0f, 32.0f);
        ...
        struct DjuiRect* rect2 = djui_rect_container_create(body, 64);
        {
            ...
            djui_base_set_enabled(&inputbox1->base, clientAvailable);
            djui_base_set_enabled(&button2->base, clientAvailable);
            defaultBase = clientAvailable ? &button2->base : &button1->base;
        }
    }
