...
static kb_callback_t on_key_down_callback;
static kb_callback_t on_key_up_callback;
static void (*on_all_keys_up_callback)(void);
static void (*on_text_input_callback)(char*);
static void (*on_text_editing_callback)(char*, int);
static void (*on_scroll_callback)(float, float);
...
static void gfx_sdl_set_keyboard_callbacks(kb_callback_t on_key_down, kb_callback_t on_key_up, void (*on_all_keys_up)(void),
    ...
    on_text_input_callback = on_text_input;
    on_text_editing_callback = on_text_editing;
}
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            ...
            case SDL_KEYUP:
                ...
                break;
            case SDL_TEXTINPUT:
                if (on_text_input_callback != NULL) {
                    on_text_input_callback(event.text.text);
                }
                break;
            case SDL_TEXTEDITING:
                if (on_text_editing_callback != NULL) {
                    on_text_editing_callback(event.edit.text, event.edit.start);
                }
                break;
            case SDL_MOUSEWHEEL:
                if (on_scroll_callback != NULL) {
                    on_scroll_callback(event.wheel.preciseX, event.wheel.preciseY);
                }
                break;
#endif
...
static void gfx_sdl_set_clipboard_text(const char *text) {
    SDL_SetClipboardText(text);
}

static void gfx_sdl_set_cursor_visible(bool visible) {
    SDL_ShowCursor(visible ? SDL_ENABLE : SDL_DISABLE);
}

static void gfx_sdl_delay(unsigned int ms) {
    SDL_Delay(ms);
}
...
static bool gfx_sdl_has_focus(void) {
    return (SDL_GetWindowFlags(wnd) & SDL_WINDOW_INPUT_FOCUS) != 0;
}
