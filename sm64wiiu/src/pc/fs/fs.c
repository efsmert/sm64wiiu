...
bool fs_init(const char *writepath) {
    ...
    // On Wii U, /vol/content contains WUHB-bundled assets. Mount it first so
    // writable SD paths can still override bundled files when both exist.
#ifdef TARGET_WII_U
    fs_mount("/vol/content");
#endif
