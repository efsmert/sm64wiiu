...
#ifndef TARGET_N64
    smlua_call_event_hooks_before_set_mario_action(m, (int)action, (int)actionArg);
#endif
...
#ifndef TARGET_N64
    smlua_call_event_hooks_on_set_mario_action(m);
#endif
...
s32 execute_mario_action(UNUSED struct Object *o) {
    ...
    if (gMarioState->action) {
        ...
        while (inLoop) {
            int luaInLoop = inLoop;
            if (smlua_call_mario_action_hook(gMarioState, &luaInLoop)) {
                inLoop = luaInLoop;
                continue;
            }
