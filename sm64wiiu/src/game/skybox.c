...
SkyboxTexture *sSkyboxTextures[] = {
    ...
    &bits_skybox_ptrlist,
    // DNC custom backgrounds (indices 10..27) extracted from dnc_skybox_geo.bin.
    &dnc_water_night_skybox_ptrlist,
    &dnc_water_sunrise_skybox_ptrlist,
    &dnc_water_sunset_skybox_ptrlist,
    &dnc_wdw_night_skybox_ptrlist,
    &dnc_wdw_sunrise_skybox_ptrlist,
    &dnc_wdw_sunset_skybox_ptrlist,
    &dnc_cloud_floor_night_skybox_ptrlist,
    &dnc_cloud_floor_sunrise_skybox_ptrlist,
    &dnc_cloud_floor_sunset_skybox_ptrlist,
    &dnc_ccm_night_skybox_ptrlist,
    &dnc_ccm_sunrise_skybox_ptrlist,
    &dnc_ccm_sunset_skybox_ptrlist,
    &dnc_ssl_night_skybox_ptrlist,
    &dnc_ssl_sunrise_skybox_ptrlist,
    &dnc_ssl_sunset_skybox_ptrlist,
    &dnc_clouds_night_skybox_ptrlist,
    &dnc_clouds_sunrise_skybox_ptrlist,
    &dnc_clouds_sunset_skybox_ptrlist,
};
...
u8 sSkyboxGlobalColor[3] = { 0xFF, 0xFF, 0xFF };
...
// Multiplies a skybox vertex channel by current Lua tint while preserving 8-bit range.
static u8 skybox_apply_global_tint(u8 base_color, u8 global_color) {
    return (u8)(((u32)base_color * (u32)global_color + 127u) / 255u);
}
...
Vtx *make_skybox_rect(s32 tileIndex, s8 colorIndex) {
    ...
    if (verts != NULL) {
        u8 r = skybox_apply_global_tint(sSkyboxColors[colorIndex][0], sSkyboxGlobalColor[0]);
        u8 g = skybox_apply_global_tint(sSkyboxColors[colorIndex][1], sSkyboxGlobalColor[1]);
        u8 b = skybox_apply_global_tint(sSkyboxColors[colorIndex][2], sSkyboxGlobalColor[2]);
        make_vertex(verts, 0, x, y, -1, 0, 0, r, g, b, 255);
        make_vertex(verts, 1, x, y - SKYBOX_TILE_HEIGHT, -1, 0, 31 << 5, r, g, b, 255);
        make_vertex(verts, 2, x + SKYBOX_TILE_WIDTH, y - SKYBOX_TILE_HEIGHT, -1, 31 << 5, 31 << 5, r, g, b, 255);
        make_vertex(verts, 3, x + SKYBOX_TILE_WIDTH, y, -1, 31 << 5, 0, r, g, b, 255);
    } else {
    }
    return verts;
}
...
Gfx *create_skybox_facing_camera(s8 player, s8 background, f32 fov,
    ...
    s8 default_background = background;
...
#ifndef TARGET_N64
    background = smlua_get_override_skybox(background);
    smlua_get_skybox_color(sSkyboxGlobalColor);
#else
    sSkyboxGlobalColor[0] = 0xFF;
    sSkyboxGlobalColor[1] = 0xFF;
    sSkyboxGlobalColor[2] = 0xFF;
#endif

    if (background < 0 || background >= (s8)ARRAY_COUNT(sSkyboxTextures)) {
