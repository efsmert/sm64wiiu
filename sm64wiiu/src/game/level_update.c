...
#ifndef TARGET_N64
    // Mirror Co-op DX hook timing: fire once local warp setup and music are ready.
    smlua_call_event_hooks_warp(HOOK_ON_WARP, warpType, sWarpDest.levelNum, sWarpDest.areaIdx,
                                sWarpDest.nodeId, (unsigned int)sWarpDest.arg);
#endif
}
...
#ifndef TARGET_N64
    // Notify Lua that level init completed with the current warp destination metadata.
    smlua_call_event_hooks_warp(HOOK_ON_LEVEL_INIT, sWarpDest.type, sWarpDest.levelNum,
                                sWarpDest.areaIdx, sWarpDest.nodeId, (unsigned int)sWarpDest.arg);
#endif
