...
static f32 geo_vec3_len(Vec3f v) {
    return sqrtf((v[0] * v[0]) + (v[1] * v[1]) + (v[2] * v[2]));
}

// Inverse for the affine, orthonormal camera matrix generated by mtxf_lookat.
// This lets downstream interpolation logic recover camera/world transforms explicitly.
static bool geo_inverse_camera_affine_orthonormal(Mat4 out, Mat4 in) {
    Vec3f xAxis = { in[0][0], in[0][1], in[0][2] };
    Vec3f yAxis = { in[1][0], in[1][1], in[1][2] };
    Vec3f zAxis = { in[2][0], in[2][1], in[2][2] };
    if (geo_vec3_len(xAxis) < 0.0001f || geo_vec3_len(yAxis) < 0.0001f || geo_vec3_len(zAxis) < 0.0001f) {
        return false;
    }
...
static void geo_process_camera(struct GraphNodeCamera *node) {
    ...
    if (geo_inverse_camera_affine_orthonormal(invCameraTransform, gMatStack[gMatStackIndex])) {
        ...
        if (invMtx != NULL) {
            mtxf_to_mtx(invMtx, invCameraTransform);
