...
8. Optional packaging helpers in this fork:
  * `make wuhb` generates `build/<VERSION>_wiiu/sm64.<VERSION>.wuhb`.
  * `make cemu-sync` copies RPX to `sm64wiiu_cemu/code/sm64.<VERSION>.rpx`.
  * `./build_wiiu.sh` runs build + WUHB + Cemu sync with devkitPro env setup.
