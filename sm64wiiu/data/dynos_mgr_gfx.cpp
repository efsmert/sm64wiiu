...
static s32 dynos_get_active_mod_index(void) {
#ifdef TARGET_WII_U
    // Wii U: no per-script active mod pointer; treat mod-data allocations as global.
    return 0;
#else
    return (gLuaActiveMod ? gLuaActiveMod->index : -1);
#endif
}
...
Gfx *DynOS_Gfx_Get(const char *aName, u32 *outLength) {
    ...
    s32 modIndex = dynos_get_active_mod_index();
