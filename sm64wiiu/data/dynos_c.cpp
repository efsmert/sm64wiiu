...
bool dynos_warp_to_start_level(void) {
    ...
    // Wii U port uses `gChangeLevelTransition` to request a level switch.
    gChangeLevelTransition = (s16) gLevelValues.entryLevel;
...
void dynos_model_init_pools(void) {
    DynOS_Model_InitPools();
}
