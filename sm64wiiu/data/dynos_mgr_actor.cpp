...
const void *DynOS_Actor_GetLayoutFromName(const char *aActorName) {
    ...
    if (is_mod_fs_file(aActorName)) {
        // Wii U: single-player build does not track a per-script "active mod" pointer yet.
        // Use mod index 0 for on-demand ModFS actor loads.
        if (DynOS_Actor_AddCustom(0, -1, aActorName, aActorName)) {
...
void DynOS_Actor_Override(struct Object* obj, void** aSharedChild) {
    ...
    if (obj && (obj->behavior == bhvMario ||
            obj->behavior == bhvNormalCap ||
            obj->behavior == bhvWingCap ||
            obj->behavior == bhvMetalCap ||
            obj->behavior == bhvVanishCap)) {
#ifndef TARGET_WII_U
...
#else
        (void)configDynosLocalPlayerModelOnly;
#endif
    }
...
void DynOS_Actor_RegisterModifiedGraphNode(GraphNode *aNode) {
    if (sModifiedGraphNodes.find(aNode) == sModifiedGraphNodes.end()) {
        struct GraphNode *sharedChild = aNode;
